...
> **Amendment (failure records — see [0044](0044-failure-records.md)).** The
> reserved per-test `issues/` directory never shipped: what it gestured at is
> now the two-level **failure record** — a step-level
> `steps/<ordinal>-<id>/failure.yaml` authored by the framework and a pack-root
> `failure.yaml` index **generated by finalize**. That also amends this
> decision's "`finalize` is UNCHANGED": finalize now has one generative output
> beyond derived totals/hashes (it writes the root index before sealing).
> References to `issues/` below are historical.
